  ...
  return (
    <header className="sticky top-0 z-10 border-b border-[var(--line)] bg-[var(--surface)]/86 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <Link href={viewer.isPreview ? "/?auth=member" : "/"} className="flex min-w-0 items-center gap-3" aria-label="Book Lens 홈">
        ...
        <nav className="flex shrink-0 items-center gap-1 rounded-full border border-[var(--line)] bg-[var(--surface-soft)] p-1 text-xs text-[var(--muted)] sm:text-sm">
          <Link href={viewer.isPreview ? "/books?auth=member" : "/books"} className="rounded-full px-3 py-2 font-bold transition hover:bg-[var(--accent-soft)] hover:text-[var(--accent-strong)]">
            동화
          </Link>
          {viewer.isMember ? (
            <>
              <Link href={viewer.isPreview ? "/chat?auth=member" : "/chat"} className="rounded-full px-3 py-2 font-bold transition hover:bg-[var(--accent-soft)] hover:text-[var(--accent-strong)]">
                내 대화
              </Link>
              <details className="group relative">
                ...
                <div className="absolute right-0 top-11 grid w-36 gap-1 rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-2 shadow-[var(--shadow)]">
                  <Link href={viewer.isPreview ? "/settings?auth=member" : "/settings"} className="rounded-2xl px-3 py-2 font-bold hover:bg-[var(--surface-soft)]">
                    마이페이지
                  </Link>
                  <form action={signOut}>
                    <button type="submit" className="w-full rounded-2xl px-3 py-2 text-left font-bold text-[var(--muted)] hover:bg-[var(--surface-soft)]">
                      로그아웃
                    </button>
                  </form>
                </div>
